...
# Trained detector model verified on the LattePanda.
YOLO_MODEL = r"D:\ava_data\animal_models\animal_detector_v14\weights\best.pt"

# Multi-animal demo classes from the trained detector.
TARGET_ANIMALS = {"coyote", "deer", "hog", "rabbit", "raccoon"}

# Smaller OpenCV overlay text for the 320x240 UI preview.
HUD_FONT = cv2.FONT_HERSHEY_SIMPLEX
HUD_STATUS_SCALE = 0.45
HUD_LABEL_SCALE = 0.38
HUD_THICKNESS = 1
...
print("[INFO] Detector model:", YOLO_MODEL)
print("[INFO] Target animals:", sorted(TARGET_ANIMALS))
...
def load_animal_classifier():
    ...
    print("[INFO] No trained animal classifier found. Using detector-only animal logic.")
    return None
...
def decide_animal_action(yolo_class, crop, classifier_model):
    # Demo behavior: alarm on the trained detector's animal classes.
    if yolo_class in TARGET_ANIMALS:
        return {
            "action": "alarm",
            "display_label": f"{yolo_class} alarm",
            "event_label": f"animal_{yolo_class}",
            "event_text": f"detector={yolo_class}",
        }

    if classifier_model is None:
        return {
            "action": "ignore",
            "display_label": f"{yolo_class} ignored",
            "event_label": None,
            "event_text": f"ignored_detector={yolo_class}",
        }
    ...
    if label in ALARM_ANIMAL_CLASSES:
        return {
            ...
            "event_text": f"classifier={label}, conf={score:.3f}, detector={yolo_class}",
        }

    return {
        "action": "ignore",
        "display_label": f"{label} ignored {score:.2f}",
        "event_label": None,
        "event_text": f"classifier={label}, conf={score:.3f}, detector={yolo_class}",
    }
            ...
            if result.boxes is not None:
                for box in result.boxes:
                    ...
                    if cls_name == "person":
                        ...
                        elif animal_decision["action"] == "alarm":
                            box_color = (0, 165, 255)
                            display_label = f"{animal_decision['display_label']} {conf:.2f}"
                            write_status("ALARM")
                            status_written = True
                            if animal_confirm_count >= ANIMAL_CONFIRM_FRAMES:
